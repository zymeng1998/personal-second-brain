...
**Phase 1A** is in progress. **SB-001, SB-002, SB-006 are `Done`** (each an atomic commit per the Atomic
Story Rule). The next story to implement is:

> **SB-003 — Create workspace tree exactly as documented** · P0 · 2 pts · `Ready` · depends on SB-002 (`Done`).

Remaining 1A order: SB-003 → SB-004 → SB-005 → SB-007, observing the 1A stop point before Phase 1B. Each
story is committed atomically after review.
