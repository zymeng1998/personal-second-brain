...
1. Capturing content writes an immutable `00_Raw/` file **and** a capture event.
2. Attempting to overwrite/delete a raw file is rejected by the guard.
3. Every note in the vault validates against the frontmatter schema.
4. A separate reader can list/read notes and read events using only documented contracts.
5. The distillation skill never edits raw and never mutates without confirmation.
6. Grep confirms **zero** domain/broker terms in `packages/`, `schemas/`, vault structure.
